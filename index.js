...
export const getProducts = skus =>
  Platform.select({
    ios: async () => RNIapIos.getItems(skus).then(items => items.filter(item => item.productId)),
...
/**
 * Finish Transaction with ID(iOS only)
 *   Explicitly call transaction finish
 * @returns {Promise<ProductPurchase>}
 */
export const finishTransactionWithTransactionIdentifier = transactionIdentifier =>
  Platform.select({
    ios: async () => RNIapIos.finishTransactionWithTransactionIdentifier(transactionIdentifier),
    android: async () => Promise.resolve()
  })();

/**
 * processAllTranscations(iOS only)
 *   Explicitly call transaction finish
 * @returns {Promise<ProductPurchase>}
 */
export const processAllTranscations = () =>
  Platform.select({
    ios: async () => RNIapIos.processAllTranscations(),
    android: async () => Promise.resolve()
  })();
...
export const addGetISOSuccPurchaseListener = e => {
  if (Platform.OS === "ios") {
    const myModuleEvt = new NativeEventEmitter(RNIapIos);
    return myModuleEvt.addListener("iap-get-succ-purchase", e);
  }
};
...
export default {
  ...
  finishTransaction,
  finishTransactionWithTransactionIdentifier,
  processAllTranscations,
  clearTransaction,
  consumePurchase,
  validateReceiptIos,
  validateReceiptAndroid,
  addAdditionalSuccessPurchaseListenerIOS,
  addGetISOSuccPurchaseListener
};
